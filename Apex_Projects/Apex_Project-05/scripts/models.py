...
class Ticket(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "event_id", name="unique_user_event"), Index("ix_ticket_status_event_id",
        "status","event_id"), # Adding a unique constraint to ensure that a user can only have one ticket for a specific event, preventing duplicate entries in the tickets table for the same user & event combination
