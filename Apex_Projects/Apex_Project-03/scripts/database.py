...
from sqlalchemy import (
    create_engine,
)  # Importing the create_engine function from SQLAlchemy to create a database engine
from sqlalchemy.orm import (
    sessionmaker,
    DeclarativeBase,
)  # Importing necessary components from SQLAlchemy for creating a database engine and session
from dotenv import (
    load_dotenv,
)  # Importing the load_dotenv function from the python-dotenv package to load environment variables from a .env file
...
# Create POSTGRES_DBg the database URL constructed from environment variables, which allows the application to connect to the PostgreSQL database
engine = create_engine(DB_URL, pool_pre_ping=True)

# Create a session factory using SQLAlchemy's sessionmaker, which will be used to create database sessions for interacting with the database
