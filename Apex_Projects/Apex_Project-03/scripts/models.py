...
class Event(Base):
    ...
    id = Column(
        Integer, primary_key=True
    )  # Primary key column for the Event model, automatically incrementing integer
    name = Column(String, nullable=False)  # Name of the event, cannot be null
    date = Column(
        DateTime, nullable=False
    )  # Date and time of the event, cannot be null
    location = Column(String, nullable=False)  # Location of the event, cannot be null

    created_at = Column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )  # Timestamp for when the event was created, cannot be null
    updated_at = Column(
        DateTime(timezone=True),
        server_default=func.now(),
        onupdate=func.now(),
        nullable=False,
    )  # Timestamp for when the event was last updated, cannot be null
    tickets = relationship(
        "Ticket", back_populates="event"
    )  # Establishing a relationship between the Event & Ticket models, allowing for easy access to an event's tickets
...
class Ticket(Base):
    ...
    )  # Price of the ticket, cannot be null
    ...
    )  # Establishing a relationship between the Ticket and User models, allowing for easy access to the user associated with a ticket
    event = relationship(
    ...
    )  # Adding a unique constraint to ensure that a user can only have one ticket for a specific event, preventing duplicate entries in the tickets table for the same user and event combination
